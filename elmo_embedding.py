...
class action_classifier:
	...
	def create_dict(self):
		...
		self.verb_classes = np.asarray(pd.read_csv('EPIC_verb_classes.csv')['class_key'])
		self.class_length = self.verb_classes.shape[0]

		verb_classes_total = np.asarray(pd.read_csv('EPIC_verb_classes.csv')['verbs'])
		...
		for i in range(len(lookup)):
			if i == 0:
				self.action_vec.append((self.verb_classes[i], action_vector[0:lookup[i], :]))
			else:
				self.action_vec.append((self.verb_classes[i], action_vector[lookup[i-1]:lookup[i], :]))

		#print(self.action_vec)
	...
	def classify(self, embeddings):
		...
		for ebd in embeddings:
			...
			for i in range(self.class_length):
				group_vec = self.action_vec[i][1]
				sum_sim = 0
				vec_count = 0

				for vec in group_vec:
					sum_sim += 1 - spatial.distance.cosine(ebd, vec)
					vec_count += 1
				
				avg_sim = sum_sim/vec_count
				if avg_sim > max_sim:
					max_sim = avg_sim
